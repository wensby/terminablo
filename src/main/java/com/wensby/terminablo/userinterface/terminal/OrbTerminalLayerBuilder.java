...
public class OrbTerminalLayerBuilder {

  private final TerminalCharacterFactory characterFactory;
  private Orb orb;
  private InterfaceSize size;

  public OrbTerminalLayerBuilder(TerminalCharacterFactory characterFactory) {
    this.characterFactory = characterFactory;
  }

  public TerminalLayer build() {
  ...
  private TerminalLayer createScaledDownRepresentation(Orb orb, InterfaceSize size) {
    TerminalLayer layer = createLayer(size);
    fillInBorder(layer);
    TerminalLayerImpl orbContent = createOrbContent(orb,
        new InterfaceSize(size.getWidth() - 2, size.getHeight() - 2));
  ...
  private TerminalCharacter getRemainderRowCharacter(BigDecimal remainder, BigDecimal rowCapacity,
      final Color color) {
    if (remainder.compareTo(BigDecimal.ZERO) == 0) {
      return characterFactory.createCharacter(' ');
    }
    BigDecimal levels = BigDecimal.valueOf(9);
    BigDecimal rowLevelCapacity = rowCapacity.divide(levels, 100, RoundingMode.UP);
    int rowLevels = remainder.divide(rowLevelCapacity, RoundingMode.CEILING).intValue();
    char c;
    if (rowLevels == 9) {
  ...
  private TerminalLayerImpl createOrbContent(Orb orb, InterfaceSize size) {
    ...
    if (fullRows != height) {
      TerminalCharacter remainderRowCharacter = getRemainderRowCharacter(remainder, rowCapacity,
          orb.getColor());
      for (int x = 0; x < size.getWidth(); x++) {
