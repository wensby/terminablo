...
public class OrbTerminalRepresentationFactory {
  ...
  private TerminalLayer createScaledDownRepresentation(Orb orb, InterfaceSize size) {
    ...
    fillInBorder(layer);
    var orbContent = createOrbContent(orb,
  ...
  private TerminalCharacter getRemainderRowCharacter(BigDecimal remainder, BigDecimal rowCapacity,
    ...
    var rowLevelCapacity = rowCapacity.divide(levels, 100, RoundingMode.UP);
    var rowLevels = remainder.divide(rowLevelCapacity, RoundingMode.CEILING).intValue();
    char c;
    if (rowLevels == 9) {
